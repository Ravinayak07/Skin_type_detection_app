...
# Header
st.set_page_config(
    page_title="Aplikasi Rekomendasi Produk Skin Care",
    page_icon=":blossom:",
    layout="wide",
)
...
category = first.selectbox(
    label="Kategori Produk : ", options=skincare["tipe_produk"].unique()
)
...
pd.DataFrame(
    tfidf_matrix.todense(), columns=tf.get_feature_names(), index=skincare.product_name
...
def skincare_recommendations(
    ...
    items=skincare[["product_name", "produk-href", "price", "description"]],
    k=5,
):
