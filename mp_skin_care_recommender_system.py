...
skincare = pd.read_csv(
    "https://drive.google.com/uc?export=download&id=19LD3liEpL9ZRi77evf5cK-icR0w_qXCx",
    low_memory=False,
)
skincare.info()
skincare.head()
...
counts_dfbrand = pd.DataFrame(
    {
        "Brand": counts_brand.index,
        "Counts": counts_brand.values,
...
fig = px.pie(
    ...
    color=pd_type.index,
    names=pd_type.index,
    color_discrete_sequence=["#003049", "#D62828", "#F77F00", "#FCBF49", "#E9D8A6"],
    hole=0.5,
)
fig.update_traces(
    textposition="outside",
    textfont=dict(color="dark blue", size=15),
    textinfo="label+percent",
    pull=[0.2, 0, 0, 0],
    rotation=10,
)
fig.add_annotation(
    text="Product Type", showarrow=False, font=dict(size=12, color="dark blue")
)
fig.update_layout(
    title={
...
fig = px.bar(
    ...
    height=600,
    text_auto=True,
    title=f"Count of Skin Type That Suitable For The Product",
)
...
counts_effect_df = pd.DataFrame(
    {
        "Notable_Effects": counts_effect.index,
        "Counts": counts_effect.values,
        "Percent%": np.round(count_percentage.values, 2),
    }
)
...
fig = px.pie(
    top_5,
    values="Counts",
    color="Notable_Effects",
    names="Notable_Effects",
    color_discrete_sequence=["#99582A", "#F4ACB7", "#FFCCD5", "#FDFCDC", "#C6AC8F"],
)
fig.update_traces(
    textposition="inside", textfont=dict(color="black", size=15), textinfo="percent"
)
fig.update_layout(
    title={
...
skincare = skincare.reindex(
    skincare.columns.tolist() + unique_skintype, axis=1, fill_value=0
)
...
def skincare_recommendations(
    ...
    items=skincare[["product_name", "notable_effects"]],
    k=5,
):
    ...
    index = similarity_data.loc[:, nama].to_numpy().argpartition(range(-1, -k, -1))
